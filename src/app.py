...
df_upper_item_equal = pd.DataFrame()
...
while isPeriod:
    ...
    for i in range(0, len(page_numbers[:-1])):
        time.sleep(3)
        driver.find_element_by_xpath(f'//*[@id="page_list"]/span[{str(i+1)}]/a').click()

    #     df_lower_item_equal = item_equal.get_list_page(driver)
    #     df_upper_item_equal = pd.concat([df_upper_item_equal, df_lower_item_equal])
...
df_upper_item_equal.reset_index(drop=True, inplace=True)
df_upper_item_equal.to_csv("data\\item_equal\\equal_items.csv", index=False, encoding="euc-kr")
